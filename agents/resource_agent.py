...
class ResourceAgent:
    ...
    def _material_request_rationale(
        ...
        storage = (
            ...
            if from_cache
            else "generated by the ResourceAgent and saved in its material cache"
        )
        why_shown = (
    ...
    def _validate_material_shape(material: object) -> dict:
        ...
        normalised = {
            f: str(material.get(f, "")).strip()
            for f in (
                *ResourceAgent._MATERIAL_REQUIRED_FIELDS,
    ...
    def _generate_materials(
        self, topic: str, course: str, student_level: str, learning_style: str
    ) -> list[dict]:
...
[
  {{
    ...
    def _generate_targeted_explanation(
        self, topic: str, course: str, student_level: str, context: str, learning_style: str
    ) -> dict | None:
...
["prerequisite 1", "prerequisite 2"]"""
    ...
    def _handle_weakness_report(self, message: AgentMessage) -> None:
        ...
        # Determine strategy
        strategy, suggestion = self._compute_strategy(topic, accuracy, attempts, weak_prereqs)
    ...
    def _compute_strategy(
        self, topic: str, accuracy: float, attempts: int, weak_prereqs: list[str]
    ) -> tuple[str, str]:
    ...
    def _detect_weakness_pattern(self) -> str | None:
        ...
        for course, topics in course_weakness.items():
            if len(topics) >= 3:
                return (
                    f"Multiple weak topics detected in {course} ({len(topics)} topics). "
        ...
        if avg_accuracy < 0.25:
            return (
                "Very low accuracy across recent topics. Consider reducing overall "
    ...
    def load_state(self, data: dict) -> None:
        ...
        for raw_key, entry in raw_kb.items():
            ...
            if key in migrated:
                ...
                existing["times_requested"] = existing.get("times_requested", 0) + int(
                    entry.get("times_requested", 0)
                )
                continue
            migrated[key] = {
                ...
                "insertion_order": int(entry.get("insertion_order", next(self._insertion_counter))),
