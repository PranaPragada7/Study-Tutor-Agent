...
# ──────────────────────────────────────────────
st.set_page_config(
    page_title="Study Tutor | Adaptive Learning",
    page_icon="✦",
    layout="wide",
)
...
if st.session_state.profile is None or st.session_state.tutor is None:
    ...
    onboarding_steps = (
        (
            ...
            "Create a private local profile and add the courses you want to focus on.",
        ),
        (
            "02",
            "Practice adaptively",
            "Work through focused quiz sessions that respond to your performance.",
        ),
        (
            "03",
            "Review with intent",
            "Use mastery signals and spaced review dates to decide what comes next.",
        ),
    )
    for column, (number, title, description) in zip(onboarding_columns, onboarding_steps):
        with column:
            st.markdown(
                f"""
                <div class="onboarding-card">
                    <span class="onboarding-number">{number}</span>
                    <h3>{title}</h3>
                    <p>{description}</p>
                </div>
                """,
